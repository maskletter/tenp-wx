...
namespace tenp {
    ...
    export const watch = ($this: any): void => {
        ...
        Object.keys(obj).forEach(name => {
            if(typeof(obj[name]) == 'object'){
                obj[name] = watchProxy($this, obj[name], name, obj, [name]);
            }
    ...
    export const watchProxy = ($this: any, obj: any, parentName: string, firstObj: any, firstNameMap: any[] = []) => {
        ...
        for(let name in obj){
            if(typeof(obj[name]) == 'object'){
                obj[name] = watchProxy($this, obj[name], name, obj, firstNameMap.concat([name]))
            }
        }
        let firstName = firstNameMap.shift();
        let dataUrl = firstNameMap.map(v => {
            if(isNaN(v)) return `.${v}`
            else return `[${v}]`
        }).join('');
        ...
        return new Proxy(obj, {
            ...
            set(obj, prop: any, value){
                if(!PrototypeKeys.has(prop)){
                    let __keys = isNaN(prop) ? `.${prop}` : `[${prop}]`
                    if(objValue[dataUrl+__keys] && objValue[dataUrl+__keys] == value) {
                        return ;
                    };
                    clearTimeout(time);
                    time = setTimeout(function(){
                        ...
                        for(var name in objValue){
                            ...
                            num[`${firstName}${name}`] = objValue[name].value;
                        }
                        $this.setData(num)
                        objValue = {};
                    },50)
                    objValue[dataUrl+__keys] =  {
                        value: value
                    }
                }
    ...
    export const switchTab = (params: { url: string }): Promise<any> => {
        return new Promise((resolve, reject) => {
            arrayEndAsync(Final_RouterBefores,'switch',params.url).then(result => {
                wx.switchTab({ url: params.url, success(result){ arrayEndAsync(resolve,'switch',params.url).then(resolve) }, fail: reject })
            })
        })
    }
    ...
    export const reLaunch = (params: { url: string }): Promise<any> => {
        return new Promise((resolve, reject) => {
            arrayEndAsync(Final_RouterBefores,'reLaunch',params.url).then(result => {
                wx.reLaunch({ url: params.url, success(result){ arrayEndAsync(resolve,'reLaunch',params.url).then(resolve) }, fail: reject })
            })
        })
    }
    ...
    export const redirectTo = (params: { url: string }): Promise<any> => {
        return new Promise((resolve, reject) => {
            arrayEndAsync(Final_RouterBefores,'redirect',params.url).then(result => {
                wx.redirectTo({ url: params.url, success(result){ arrayEndAsync(resolve,'redirect',params.url).then(resolve) }, fail: reject })
            })
        })
    }
    ...
    export const navigateTo = (params: { url: string }): Promise<any> => {
        return new Promise((resolve, reject) => {
            arrayEndAsync(Final_RouterBefores,'navigate',params.url).then(result => {
                wx.navigateTo({ url: params.url, success(result){ arrayEndAsync(resolve,'navigate',params.url).then(resolve) }, fail: reject })
            })
        })
    }
     ...
     */
    export const navigateBack = (params: { delta: number }): Promise<any> => {
        return new Promise((resolve, reject) => {
            wx.navigateBack({ delta: params.delta, success(result){ arrayEndAsync(resolve,'back').then(resolve) }, fail: reject })
        })
    }
