...
(function (tenp) {
            ...
            Object.defineProperty($this.data, name, {
                ...
                set: function (value) {
                    ...
                    if (typeof (value) == 'object') {
                        obj[name] = tenp.watchProxy($this, value, name, obj, [name]);
                    }
                    else {
                        obj[name] = value;
                    }
    ...
    tenp.watchProxy = function ($this, obj, parentName, firstObj, firstNameMap) {
        if (firstNameMap === void 0) { firstNameMap = []; }
        var systemWatchEvent = $this.systemWatchEvent();
        var objValue = {};
        var time = null;
        for (var name_1 in obj) {
            if (typeof (obj[name_1]) == 'object') {
                obj[name_1] = tenp.watchProxy($this, obj[name_1], name_1, obj, firstNameMap.concat([name_1]));
            }
        }
        var firstName = firstNameMap.shift();
        var dataUrl = firstNameMap.map(function (v) {
            if (isNaN(v))
                return "." + v;
            else
                return "[" + v + "]";
        ...
        return new Proxy(obj, {
            get: function (target, name) {
                if (PrototypeKeys.has(name))
                    return target[name];
                var formatName = isNaN(name) ? "." + name : "[" + name + "]";
                var watch = systemWatchEvent[firstName + dataUrl + formatName];
                if (watch) {
                    $this[watch]({ type: 'get' });
                }
                return target[name];
            ...
            set: function (obj, prop, value) {
                if (!PrototypeKeys.has(prop)) {
                    var __keys_1 = isNaN(prop) ? "." + prop : "[" + prop + "]";
                    if (objValue[dataUrl + __keys_1] && objValue[dataUrl + __keys_1] == value) {
                        return;
                    }
                    ;
                    clearTimeout(time);
                    time = setTimeout(function () {
                        var num = {};
                        for (var name in objValue) {
                            var watch_1 = systemWatchEvent[firstName + dataUrl + __keys_1];
                            if (watch_1) {
                                $this[watch_1]({ type: 'set', value: objValue[name].value });
                            }
                    ...
                    objValue[dataUrl + __keys_1] = {
                        value: value
                    };
                }
                if (typeof (obj[prop]) == 'object') {
                    obj[prop] = tenp.watchProxy($this, obj[prop], prop, obj, firstNameMap.concat([prop]));
                }
                else {
                    obj[prop] = value;
                }
