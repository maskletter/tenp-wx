...
var FormatMethod = new Map([
    ["App", null],
    ["Page", null],
    ["Component", ComponentSetMethod.concat(['lifetimes', 'observers', 'properties', 'attached', 'detached', 'created', 'attached', 'ready', 'moved', 'detached', 'error', 'pageLifetimes', 'show', 'hide', 'resize'])],
]);
            ...
            for (var key in value) {
                if (wx_key.indexOf(key) != -1) {
                    if (key == 'for' || key == 'if' || key == 'elif')
                        wxml += ' qq:' + key + '="{{' + qE(value[key]) + '}}"';
                    else
                        wxml += ' qq:' + FormatLabel(key) + '="' + value[key] + '"';
                }
