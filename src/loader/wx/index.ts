...
function parse(content: string, rootDir: string){
	...
	parseContent.body.forEach((data: any) => {
		...
		if(data.type == 'ExpressionStatement'){
			...
			if(declaration.id.name != packageName){
				...
				wtsConfig.tree(type, value, rootDir)
				tree.push({
					type: 'main',
					value: value
				})
...
export default  (content: string, rootDir: string) => {
	...
	if(wxml){
		wxml = wxml.replace(/{{(.+?)\|(.+?)}}/g, function(a:string,b:string,c:string){
			const fun = c.split(':');
